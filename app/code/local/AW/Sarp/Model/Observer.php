        ...
        if (!Mage::getSingleton('sarp/subscription')->getId()) {
            ...
            foreach ($items as $item)
            {
                if (Mage::helper('sarp')->isSubscriptionType($item)) {
                    ...
                    else if(is_string($Options->getAwSarpSubscriptionStart()) && trim($Options->getAwSarpSubscriptionStart()) === ''){
                        $date = new Zend_Date(null, Mage::app()->getLocale()->getDateFormat(Mage_Core_Model_Locale::FORMAT_TYPE_SHORT));
                    } else {
        ...
        if (!Mage::getSingleton('sarp/subscription')->getId() && $haveSarpItems) {
            foreach ($quote->getAllItems() as $item) {
                ...
                if (Mage::helper('sarp')->isSubscriptionType($item) && !is_null($sarpSubscriptionType)) {
                    ...
                    if (($date->compare($today, Zend_Date::DATE_SHORT) < 0
                            || !$period->isAllowedDate($date, $item->getProduct()))
                    ) {
            ...
            switch($order->getPayment()->getMethod()) {
                case AW_Sarp_Model_Payment_Method_Authorizenet::PAYMENT_METHOD_CODE:
                    $paymentModel = Mage::getModel('sarp/payment_method_authorizenet'); //hardcode because unable get p.method without subscription
