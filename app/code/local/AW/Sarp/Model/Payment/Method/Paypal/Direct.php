...
class AW_Sarp_Model_Payment_Method_Paypal_Direct extends AW_Sarp_Model_Payment_Method_Paypal_Abstract
{
     ...
     */
    protected function _initWebService($storeId = null)
    {
        $service = Mage::getModel(self::WEB_SERVICE_MODEL);
        $service
                ->setAPIUsername(Mage::getStoreConfig(self::XML_PATH_PP_API_USERNAME, $storeId))
                ->setAPIPassword(Mage::getStoreConfig(self::XML_PATH_PP_API_PASSWORD, $storeId))
                ->setAPISignature(Mage::getStoreConfig(self::XML_PATH_PP_API_SIGNATURE, $storeId))
                ->setAPIEndpoint(
                    Mage::getStoreConfig(self::XML_PATH_PP_DIRECT_SANDBOX_FLAG, $storeId) ? self::ENDPOINT_SANDBOX : self::ENDPOINT_PRODUCTION
                );
    ...
    public function isValidForTransaction(AW_Sarp_Model_Sequence $Sequence)
    {
        ...
        $this->_initWebService($Subscription->getStoreId());
        $result = $this->getSubscriptionDetails($Subscription);

        if (isset($result['PROFILEID']) && isset($result['STATUS'])) {
    ...
    public function createSubscription(AW_Sarp_Model_Subscription $Subscription, $Order, $Quote)
    {
        ...
        $no = Mage::getModel('sarp/order_create')->reset();
        $no
                ->setRecollect(1);
