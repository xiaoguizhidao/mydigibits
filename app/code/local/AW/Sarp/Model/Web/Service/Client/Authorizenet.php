...
class AW_Sarp_Model_Web_Service_Client_Authorizenet extends AW_Sarp_Model_Web_Service_Client
{
    ...
    public function getPaymentAction()
    {
        ...
        if($this->getSubscription() && $this->getSubscription()->getStoreId())
            $_storeId = $this->getSubscription()->getStoreId();
        return Mage::getStoreConfig(AW_Sarp_Model_Payment_Method_Authorizenet::XML_PATH_AUTHORIZENET_PAYMENT_ACTION, $_storeId);
    }
                               ...
                               'transaction' => array(
                                   'profileTransAuthOnly' => array(
                                      'amount' => $this->getOrder()->getGrandTotal(),
                                       'tax' => array (
                                           'amount' => $this->getOrder()->getBaseTaxAmount()
                                       ),
                                       'shipping' => array (
                                           'amount' => $this->getOrder()->getBaseShippingAmount()
                                       ),
                                       'customerProfileId' => $this->getSubscription()->getRealId(),
                                       'customerPaymentProfileId' => $this->getSubscription()->getRealPaymentId(),
                                       'order' => array(
        ...
        try {
            ...
            if ($result) {
                ...
                if (strtolower($result->$resultName->resultCode) == 'error') {
                    ...
                    throw new SoapFault(
                        null,
                        "Authorize.net[{$resultName}] responded error #{$result->$resultName->messages->MessagesTypeMessage->code} '{$result->$resultName->messages->MessagesTypeMessage->text}'",
                        null,
                        $data
                    );
                    return false;
