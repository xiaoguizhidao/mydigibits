 ...
 */
class EM_Blog_Model_Resource_Post_Link_Post_Collection extends EM_Blog_Model_Resource_Post_Collection
{
    /**
     * Store product model
     *
     * @var EM_Blog_Model_Post
     */
    protected $_post;
     ...
     *
     * @param EM_Blog_Model_Post_Link $linkModel
     * @return EM_Blog_Model_Resource_Post_Link_Post_Collection
     */
    public function setLinkModel(EM_Blog_Model_Post_Link $linkModel)
    {
     ...
     */
    public function setProduct(EM_Blog_Model_Post $product)
    {
        $this->_post = $product;
    ...
    public function addExcludeProductFilter($products)
    {
        if (!empty($products)) {
            ...
            $this->_hasLinkFilter = true;
            $this->getSelect()->where('links.linked_post_id NOT IN (?)', $products);
        }
        return $this;
    }
    ...
    public function addProductFilter($products)
    {
        if (!empty($products)) {
            ...
            $this->getSelect()->where('links.post_id IN (?)', $products);
            $this->_hasLinkFilter = true;
        }
    ...
    protected function _joinLinks()
    {
        ...
        $joinCondition = array(
            'links.linked_post_id = e.entity_id',
        ...
        if ($this->getProduct() && $this->getProduct()->getId()) {
            ...
            if ($this->_isStrongMode) {
                $this->getSelect()->where('links.post_id = ?', (int)$productId);
            } else {
                $joinType = 'joinLeft';
                $joinCondition[] = $adapter->quoteInto('links.post_id = ?', $productId);
            }
        ...
        if($this->_hasLinkFilter) {
            $select->$joinType(
                array('links' => $this->getTable('blog/post_link')),
    ...
    public function joinAttributes()
    {
        ...
        foreach ($attributes as $attribute) {
            ...
            $joinCondiotion = array(
                ...
                $this->getSelect()->getAdapter()->quoteInto("{$alias}.post_link_attribute_id = ?", $attribute['id'])
            );
