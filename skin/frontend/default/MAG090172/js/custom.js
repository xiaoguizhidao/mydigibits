...
$k(document).ready(function(){
    ...
    $k('.show-amfinder').click(function(){
        if($k('.amfinder').css('display') == "block"){
            $k('.amfinder').css('display', 'none');
        }
        else{
            $k('.amfinder').css('display', 'block');
        }

        window.scrollTo(0,0);
    });

    if($k(window).width() >= 980 && document.body.hasClassName('cms-home')){
        $k('.padder .customNavigation').remove();
        $k(".padder .product-carousel").each(function(){
            var objectID = $k(this).attr('id');
            var objectClass = $k(this).attr('class');

            if(objectID.length >0){
                this.setAttribute('id', objectID.replace('-carousel', '-grid'));
            }
            if(objectClass.length >0){
                this.classList.remove(objectClass);
                this.classList.add(objectClass.replace('-carousel', 's-grid'));
            }
        });
    }

    $k(".search-selector").on("keyup click input", function () {
		if (this.value.length > 0) {
			var key = $k(this).val();
			var selectors = $k(this).parent().parent().find('dd');
			
			selectors.show();
			selectors.filter(function () {
				return $k(this).find('label').text().toLowerCase().indexOf(key.toLowerCase()) == -1;
			}).hide();
		}
		else {
		  	$k(this).parent().parent().find('dd').show();
		}
	});
...
$k(window).resize(function(){productCarouselAutoSet();});
...
$k(document).ready(function(){productListAutoSet();});
...
function composeDefaultMenu(){
    $$("#nav li > ul.level0").each(function(category){
        var width = 0;
        category.childElements().each(function(li){
            if(li.getElementsByTagName('ul').length != 0){
                width += li.getElementsByTagName('ul')[0].getWidth();
                li.style.width = li.getElementsByTagName('ul')[0].getWidth() + "px";
            }
            else{
        ...
        category.style.width = width + "px";
    });
}
$k(document).ready(function(){composeDefaultMenu();});
$k(window).resize(function(){composeDefaultMenu();});

function listHeightAutoSet() {
    var height = 0;
    $k("ul.list-grid").each(function(){
        $k(this).children().each(function(){
            if(height < $k(this).find('li').height()){height = $k(this).find('li').height();}
        })

        $k(this).children().each(function(){
            $k(this).find('li').height(height);
        });
    });
}
$k(document).ready(function(){listHeightAutoSet();});
$k(window).resize(function(){listHeightAutoSet();});
